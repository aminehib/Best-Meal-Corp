...
$(document).ready(function() {
 let ingredientSelect = $('#ingredient-select');
 let tagSelect = $('#tag-select');

 ingredientSelect.selectize({
    create: true,
 ...
 for (i = 0; i < buttons.length; i++) {
  buttons[i].addEventListener('click', function(e) {
   let formId = this.getAttribute('data-target');
   let formBlock = document.querySelector('[data-form-id="' + formId + '"]');
   let firstInput = null;
   ...
   if (formBlock.classList.contains('is-hidden')) {
    ...
    this.textContent = 'x';
    firstInput = formBlock.querySelector('input, textarea');
